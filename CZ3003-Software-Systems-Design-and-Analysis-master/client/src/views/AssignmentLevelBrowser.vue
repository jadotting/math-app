...
export default {
  name: 'AssignmentLevelBrowser',

  props: { 'assignment_world': String, 'assignment_level': String },
  ...
  mounted() {
    // Form navTiles for every matching assignment question set
    let incrementedId = 0;
    this.allAssignments.forEach((assignment) => {
      if (assignment.world === this.assignment_world && assignment.level === this.assignment_level) {
        this.navTiles.push({
          id: incrementedId,
          title: assignment.title.split('_').join(' '),
          to: `/assignments/${assignment.world}/${assignment.level}/${assignment.id}`,
          description: '',
          reqExp: assignment.reqExp,
        });
        incrementedId += 1;
      }
    });
