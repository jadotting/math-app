<template>
  <div class='text-center h-100' id='assignment-browser'>
    ...
    <b-row class='h-100' align-v='start' no-gutters>
      ...
      <b-col v-else cols='12' class='text-center align-self-center' style='color: white' >
        ...
        <b-button to='/assignments' pill variant='warning'>
          Back to Safety
        </b-button>
      </b-col>
    </b-row>
    ...
    >
      <div class='d-block text-center'>
        <p class='mb-3 text-accent'>You have completed this assignment!</p>
...
export default {
  ...
  props: { 'assignment_world': String, 'assignment_level': String, 'assignment_id': String },
  ...
  async beforeMount() {
    ...
    // Find the matching assignment question set based on assignment_id
    if (this.allAssignments.hasOwnProperty('length')) {
      this.toRender = this.allAssignments.find((assignment) => {
        return assignment.id === this.assignment_id;
      });
    } else {
      await this.$store.dispatch('getQuestionSetById', {
        type: 'assignments',
        world: this.assignment_world,
        level: this.assignment_level,
    ...
    // Find the matching result using assignment_id first, if not found, then create placeholders
    if (this.userData.hasOwnProperty('results')) {
      for (let [key, value] of Object.entries(this.userData.results)) {
        if (value.id === this.assignment_id) {
          this.result = value;
  ...
  computed: {
    ...
    twitterShareString: {
      get() {
        ...
        return 'https://twitter.com/intent/tweet?text=I%20completed%20an%20assignment%20on%20' +
